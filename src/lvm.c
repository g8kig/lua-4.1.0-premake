...
int call_binTM (lua_State *L, const TObject *p1, const TObject *p2,
...
int luaV_equal (lua_State *L, const TObject *l, const TObject *r) {
  int teq = 0;
  int typ = ttype(l);
  if (typ == ttype(r)) {
    switch(typ) {
  ...
  /* try TM */
  if (call_binTM(L, l, r, L->top, TM_EQ))
    return (ttype(L->top) != LUA_TNIL);
  else if (teq == 1)
    return tsvalue(l) == tsvalue(r);
  /* existing behaviour */
  ...
  for (;;) {
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_TESTEQ: {
        ...
        if (luaV_equal(L, ra, RKC(i))) dojump(pc, *pc);
        pc++;
        break;
      }
      case OP_TESTNE: {
